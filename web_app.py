...
def calculation_page():
    ...
    with cols[-1]:
        ...
        final_readings['water'] = st.number_input(
            "Final Reading", 
            min_value=initial_readings['water'], 
            value=initial_readings['water'],
            step=0.1, 
            key="water_final"
        )
        st.markdown('</div>', unsafe_allow_html=True)
...
def history_page():
    ...
    with col1:
        ...
        st.download_button(
            ...
            data=json_data,
            file_name=f"electricity_history_{datetime.now().strftime('%Y%m%d')}.json",
    ...
    with col2:
        ...
        if excel_data:
            st.download_button(
                label="📊 Export to Excel",
                data=excel_data,
                file_name=f"{st.session_state.settings['compound_name']}_electricity_history_{datetime.now().strftime('%Y%m%d')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            )
...
def settings_page():
            ...
            if history:
                ...
                if excel_data:
                    st.download_button(
                        ...
                        data=excel_data,
                        file_name=f"{st.session_state.settings['compound_name']}_history_{datetime.now().strftime('%Y%m%d')}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                    )
            else:
    ...
    st.markdown(f"""
    **{compound_name} Electricity Tracker** helps you:
...
def customization_page():
    ...
    with col1:
        ...
        st.download_button(
            ...
            data=settings_json,
            file_name=f"{settings['compound_name']}_settings_{datetime.now().strftime('%Y%m%d')}.json",
            mime="application/json"
        )
